...
Field::Field() { 
    ...
    this->checked = new bool[this->size];

    for (int i = 0; i < this->size; i++) {
        this->cells[i] = 0;
    }

    int count = this->num_mines;
    while (count > 0) {
        int rand_idx = rand() % (this->size);
        if (this->cells[rand_idx] != 1) {
            this->cells[rand_idx] = 1;
            count--;
        }
    }
...
Field::Field(int size, int num_mines) {
    this->size = (size > 3) ? size : 7;
    this->num_mines = (num_mines >= 2) ? num_mines : 2;
...
string Field::answer_string() const {
    ...
    for (int i = 0; i < size; i++) {
        if (cells[i] == 1) {
            ...
            int neighbor_count = neighbor_mines(i);
            if (neighbor_count == 0) str += "     |";
            else str += "  " + std::to_string(neighbor_count) + "  |";
        }
    }
    str += "\n";
...
int Field::neighbor_mines(int index) const {
    ...
    int count = 0;
    if (index > 0) count += cells[index-1];
    if (index < size - 1) count += cells[index+1];
